...
if (loadPlotBool & !futureBool){
  HedgeCommodityPloty <- plot_ly(x=~gridData$timestamp,y=~gridData$Load,type='scatter',mode='lines',name='Commodity Load') %>% add_lines(y=~gridData$HedgePower,type='scatter',mode='lines',name='Hedge Power') %>% layout(xaxis=list(title='Time'),yaxis=list(title='Load'))
