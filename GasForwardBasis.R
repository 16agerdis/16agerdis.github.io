...
GasForward <- read_excel("Z:/IT/Summer Intern 2018 Projects/R Project/RScripts/Gas/GasForward.xlsx")
#View(GasForward)
...
forward_gas <- plot_ly(x = ~Year, y=~Price_NYMEX,type = 'scatter',mode='lines+markers', name= 'NYMEX Henry Hub')  %>%
  ...
  layout(yaxis = y, title = "Price(USD)/Dth vs. Year (For Future Months)") 
