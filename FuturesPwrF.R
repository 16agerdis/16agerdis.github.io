...
zema4Query <- paste("select t.CONTRACT_YEAR, t.CONTRACT_MONTH, t.SETTLEMENT_PRICE, t.HUB 
                    ...
                    where trade_date = date '", userDate, "' and CONTRACT_YEAR <= ",as.numeric(substr(as.Date(userDate),1,4))+3, " and hub = 'NYISO F' and product = 'Peak Futures (50 MW)'", sep="")

zema4 <- sqlQuery(ch3, zema4Query)

zema5Query <- paste("select t.CONTRACT_YEAR, t.CONTRACT_MONTH, t.SETTLEMENT_PRICE, t.HUB 
                    from ze_data.ice_cleared_power_settle t 
                    where ((CONTRACT_YEAR = ", userYear, " and CONTRACT_MONTH >= ", userMonth, ") or CONTRACT_YEAR > ",userYear,") and trade_date = date '", userDate2, "' and CONTRACT_YEAR <= ",as.numeric(substr(as.Date(userDate),1,4))+3, " and hub = 'NYISO F' and product = 'Peak Futures (50 MW)'", sep="")
...
fn.Price <- zema4$SETTLEMENT_PRICE
...
fo.Price <- zema5$SETTLEMENT_PRICE

FPF.Month <- zema4$Month

p <- plot_ly(x=~FPF.Month, y=~fn.Price, type = "scatter", mode= "lines", name = 'Futures(Current)') %>%
  add_trace(y=~fo.Price, type="scatter", mode="lines", name= 'Futures(30Days-Prior)')

der <- (fn.Price - fo.Price)

d <- plot_ly(x=~FPF.Month, y=~der, type = "scatter", mode= "lines",name='Change in Futures')

sp <- subplot(p,d, shareX = TRUE, nrows = 2) %>% layout(title= 'Zone F: Electric Futures vs Futures (30 Days Prior)',xaxis= list(title='Time (Year)'),yaxis = list(title = 'Price (USD)/Dth'), yaxis2 =list(title = 'Change in Price (USD)/Dth'))
